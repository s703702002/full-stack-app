...
export async function getUserMedias(req: Request, res: Response) {
  ...
  const page = parsePositiveInt(asSingleString(req.query.page), 1);
  const limit = Math.min(
    100,
    parsePositiveInt(asSingleString(req.query.limit), 30),
  );
  const mediaType = parseMediaType(asSingleString(req.query.type));
